 ...
 * Solución al error de carga de tablas usando el ID de publicación correcto y la URL de la API.
 */
...
function initDashboard() {
    ...
    SHEET_NAMES.forEach(sheetName => {
        // Formato de URL CORRECTO para la API de Visualización (usando /d/e/ID/gviz/tq)
...
// Lógica Matemática y Renderizado (omitida por brevedad, se mantiene igual)
...
function calculateMetrics(data) {
    ...
    const processedData = data.map(item => {
        ...
        return { ...item, recognized: recognizedValue, isCapped: isCapped, lost: lostLikes };
    });
...
function renderDashboard() {
    ...
    dashboardData.forEach((data, index) => {
        renderCard(data.name, data.metrics, index === 0);
    });
    if (dashboardData.length === 0) {
         showError("Datos no encontrados", "Verifica que las hojas de Google Sheets tengan datos válidos.");
    }
}
...
function renderCard(name, metrics, isWinner) {
    ...
    card.innerHTML = `
        ...
        <div class="metrics-grid">
            <div class="metric-item full-width" data-tooltip="...">
                <div>
            ...
            <div class="metric-item" data-tooltip="...">
...
// Las funciones showModal, closeModal, showError y los listeners de tooltip se mantienen igual.
